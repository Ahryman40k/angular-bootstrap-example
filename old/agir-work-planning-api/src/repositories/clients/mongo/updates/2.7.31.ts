...
const logger = createLogger('mongo/2.7.31');

/**
 * For V2.7.31 we need to update the project's drm number fields from a integer to a string
 */
...
export default async function update(db: MongoDb.Db): Promise<void> {
  try {
    ...
    logger.info(`Script 2.7.31 executed in ${milliseconds} milliseconds`);
  } catch (e) {
