...
const logger = createLogger('mongo/2.8.13');
let INTERVENTIONS_COLLECTION: MongoDb.Collection;

/**
 * For V2.8.13 to remove geometry from assets in no-geolocated Interventions
 */
...
export default async function update(db: MongoDb.Db): Promise<void> {
  try {
    ...
    logger.info(`Script 2.8.13 executed in ${milliseconds} milliseconds`);
  } catch (e) {
