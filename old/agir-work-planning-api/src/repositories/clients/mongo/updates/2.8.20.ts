...
const logger = createLogger('mongo/2.8.20');
let TAXONOMIES_COLLECTION: MongoDb.Collection;

/**
 * For V2.8.20 we need to modify the taxonomy group requirementSubtype
 */
export default async function update(db: MongoDb.Db): Promise<void> {
  ...
  logger.info(`Script 2.8.20 executed in ${milliseconds} milliseconds`);
}
